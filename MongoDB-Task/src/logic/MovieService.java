...
//http://mongodb.github.io/mongo-java-driver/2.13/getting-started/quick-tour-admin/
...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		// TODO: Select database "imdb" -> done
		db = mongo.getDB("imdb");
		...
		// TODO: Take "movies" and "tweets" collection -> done
		...
		// TODO: Index Movie attributes "title", "rating", "votes", "tweets.coordinates" -> done
		...
		//print first object of each collection
		DBObject movie1 = movies.findOne();
		System.out.println(movie1);
		DBObject tweet1 = tweets.findOne();
	...
	public DBCursor getBestMovies(int minVotes, double minRating, int limit) {
		...
		//create list for different fields and specify conditions
		List<BasicDBObject> fields = new ArrayList<BasicDBObject>();
		fields.add(new BasicDBObject("votes", new BasicDBObject("$gte", minVotes)));
		fields.add(new BasicDBObject("rating", new BasicDBObject("$gte", minRating)));
		//add to query
		query.put("$and", fields);
		//execute query and sort cursor
		DBCursor best = movies.find(query);
		best.sort(new BasicDBObject("rating", -1));
		//set limit of elements to be returned
		best.limit(limit);
		return best;
	}
	...
	public DBCursor getByGenre(String genreList, int limit) {
		...
		for(String gen:genres)
			fields.add(new BasicDBObject("genre", gen));
		//add to query
		query.put("$and", fields);
		//execute query and sort cursor
	...
	public DBCursor searchByPrefix(String titlePrefix, int limit) {
		//TODO: implement -> done
		//specify query, use "$regex" and "^" to search for prefix, "$option" "i" for case insensitivity
		BasicDBObject prefixQuery = new BasicDBObject("title", new BasicDBObject("$regex", "^"+titlePrefix).append("$options", "i"));
